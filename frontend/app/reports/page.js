        ...
        {activeReport === "sales" && (
          <div>
            <div className="bg-green-100 p-3 rounded mb-3">
              <strong>Total Sales: Rs.{salesData.total.toFixed(2)}</strong>
            </div>
        ...
        {activeReport === "purchases" && (
          <div>
            <div className="bg-red-100 p-3 rounded mb-3">
              <strong>Total Purchases: Rs.{purchaseData.total.toFixed(2)}</strong>
            </div>
            <table className="w-full border-collapse border border-gray-300">
              <thead>
                <tr className="bg-gray-100">
        ...
        {activeReport === "stock" && (
          <div>
            <div className="bg-blue-100 p-3 rounded mb-3">
              <strong>Total Stock Value: Rs.{stockData.total_stock_value.toFixed(2)}</strong>
            </div>
            <table className="w-full border-collapse border border-gray-300">
              <thead>
                <tr className="bg-gray-100">
              ...
              <tbody>
                {stockData.items.map((i) => (
                  <tr key={i.id} className={i.status === "Low Stock" ? "bg-red-50" : ""}>
                    ...
                    <td className="border border-gray-300 p-2">Rs.{i.price}</td>
                    <td className="border border-gray-300 p-2">{i.quantity}</td>
                    <td className="border border-gray-300 p-2">Rs.{i.stock_value.toFixed(2)}</td>
        ...
        {activeReport === "customers" && (
          <div>
            <div className="bg-yellow-100 p-3 rounded mb-3">
              <strong>Total Outstanding: Rs.{customerData.total_outstanding.toFixed(2)}</strong>
            </div>
            <table className="w-full border-collapse border border-gray-300">
              <thead>
                <tr className="bg-gray-100">
