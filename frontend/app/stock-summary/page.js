...
export default function StockSummary() {
  ...
  useEffect(() => {
    ...
    fetch("http://127.0.0.1:5000/api/stock_summary?company_id=" + companyId, {
        ...
        <table className="w-full border-collapse border border-gray-400">
          <thead>
            <tr className="bg-gray-700 text-white">
              ...
              <th className="border border-gray-400 p-2 text-center">Inwards (Purchased)</th>
              <th className="border border-gray-400 p-2 text-center">Outwards (Sold)</th>
            ...
            {stocks.map((s, i) => (
              <tr key={i} className={s.closing < 10 ? "bg-red-50" : "hover:bg-gray-50"}>
                ...
                <td className="border border-gray-300 p-2 text-center font-bold">
                  <span className={s.closing < 10 ? "text-red-600" : "text-green-700"}>
                    {s.closing} units
                  </span>
                </td>
                <td className="border border-gray-300 p-2 text-center text-blue-600">
                  Rs.{s.purchase_value.toFixed(2)}
                </td>
                <td className="border border-gray-300 p-2 text-center text-green-600">
                  Rs.{s.sale_value.toFixed(2)}
                </td>
                <td className="border border-gray-300 p-2 text-center">
                  <span className={"px-2 py-0 rounded text-xs font-bold " +
                    (s.closing < 10
          ...
          <tfoot>
            <tr className="bg-gray-200 font-bold">
              ...
              <td className="border border-gray-300 p-2 text-center text-blue-700">
                {stocks.reduce((sum, s) => sum + s.inwards, 0)} units
              </td>
              <td className="border border-gray-300 p-2 text-center text-orange-700">
                {stocks.reduce((sum, s) => sum + s.outwards, 0)} units
              </td>
              <td className="border border-gray-300 p-2 text-center">
                {stocks.reduce((sum, s) => sum + s.closing, 0)} units
              </td>
              <td className="border border-gray-300 p-2 text-center text-blue-600">
                Rs.{stocks.reduce((sum, s) => sum + s.purchase_value, 0).toFixed(2)}
              </td>
              <td className="border border-gray-300 p-2 text-center text-green-600">
                Rs.{stocks.reduce((sum, s) => sum + s.sale_value, 0).toFixed(2)}
              </td>
